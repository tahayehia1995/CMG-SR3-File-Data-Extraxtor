...
def _extract_single_file_spatial(args):
    ...
    """
    case_idx, sr3_file, filename, file_lookup, target_days, selected_properties, grid_dims, active_cell_mapping, inactive_value = args
    ...
    for prop in selected_properties:
        result_data[prop] = np.full((n_timesteps, nx, ny, nz), inactive_value, dtype=np.float32)
    ...
    try:
        with h5py.File(sr3_file, 'r') as f:
            for ts_idx, target_day in enumerate(target_days):
                ...
                if not plan_entry:
                    # Fill with inactive cell value for this timestep
                    for prop in selected_properties:
                ...
                for prop in selected_properties:
                    if prop in spatial_group:
                        ...
                        result_data[prop][ts_idx, :, :, :] = grid_3d
                    else:
        ...
        # Fill with inactive cell value for this case
        for prop in selected_properties:
...
class BatchSR3Extractor:
    ...
    def batch_extract_spatial_properties(self, selected_properties, selected_date_labels, output_prefix):
        ...
        try:
            ...
            print(f"📊 Batch format: ({n_cases}, {n_timesteps}, {nx}, {ny}, {nz})")
            
            # Get user-specified inactive cell value
            inactive_value = self.inactive_cells_value_widget.value
            
            # Initialize data arrays for each property using inactive cell value
            batch_data = {}
            for prop in selected_properties:
                batch_data[prop] = np.full((n_cases, n_timesteps, nx, ny, nz), inactive_value, dtype=np.float32)
            ...
            for prop in selected_properties:
                ...
                self._save_batch_spatial_to_hdf5(batch_data[prop], prop, metadata_timesteps, output_file, target_days)
    ...
    def _save_batch_spatial_to_hdf5(self, data_array, property_name, timesteps, output_file, target_days):
