...
class StreamlitSR3Extractor:
    ...
    def extract_spatial_properties(self, selected_properties, selected_date_labels, output_folder, 
        ...
        # Set selected layers (convert to tuple for widget compatibility)
        if selected_layers is None:
            selected_layers = []
        if hasattr(self.extractor, 'spatial_layers_widget'):
            # Convert layer indices to widget values (which are strings like "Layer 0")
            if selected_layers:
                layer_widget_values = tuple(f"Layer {i}" for i in selected_layers)
                self.extractor.spatial_layers_widget.value = layer_widget_values
            else:
                self.extractor.spatial_layers_widget.value = ()  # Empty = all layers
